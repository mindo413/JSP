...
<body>
	...
	<br> 
	medium : <fmt:formatDate value="${now }" type="date" dateStyle="medium" />
	<br> 
	...
	default : <fmt:formatDate value="${now }" type="time" timeStyle="default" />
	<br> 
	short : <fmt:formatDate value="${now }" type="time" timeStyle="short" />
	<br> 
	medium : <fmt:formatDate value="${now }" type="time" timeStyle="medium" />
	<br> 
	...
	<br> 
	HH:mm:ss.S : <fmt:formatDate value="${now }" pattern="HH:mm:ss.S" />
	<br> 
	KK:mm:ss : <fmt:formatDate value="${now }" pattern="KK:mm:ss" />
	<br>
	<hr>
	
	<%-- 날짜/시간 입력한 변수 만들기 --%>
	<c:set var="testTime" value="<%=new GregorianCalendar(2020, 0, 1, 0, 0, 0).getTime()%>" />
	...
	오전/오후 : <fmt:formatDate value="${now }" pattern="a" />
	<br>
	
	z : <fmt:formatDate value="${now }" pattern="z" />
	<br>
	Z : <fmt:formatDate value="${now }" pattern="Z" />
	<br>
	X : <fmt:formatDate value="${now }" pattern="X" />
	<br>
